...
**运营价值链本体（进销存 + 运维）** —— 中庸之道，不过度复杂：
- 覆盖：采购 / 生产 / 库存 / 销售 / 财务(回款) / 设备运维 六个运营域
- 9 实体类型（产品/供应商/库存/销售/客户/设备/采购/生产/回款）+ 类别类层级
- 8 决策指标（补货/缺货/呆滞/比价/供应商/预测/信用/维护）
- **明确不在范围**（设计取舍）：组织/人力、深层财务(现金流优化/预算)、优化型决策(排程/定价)、需求预测(ML)——这些需要更大数据+ML，破坏轻量定位，留作演进

**为何此范围**：立足中小，进销存+运维覆盖 80% 日常运营痛点（缺货/断供/积压/账期/设备趴窝），可落地零 token 确定性。
